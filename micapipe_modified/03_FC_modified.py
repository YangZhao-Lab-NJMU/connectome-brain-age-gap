...
# ------------------------------------------
# Subcortex
sctx = np.loadtxt(funcDir+'/volumetric/' + subject + func_lab + '_timeseries_subcortical.csv')
#写出sctx为csv
np.savetxt(funcDir+'/volumetric/' + subject + func_lab + '_timeseries_subcortical1.csv', sctx, delimiter=',')
if sctx.shape:
    n_sctx = sctx.shape[1]
else:
...
#del sctx
#del cereb
sctx_corr = get_regressed_data(x_spike, sctx, performNSR, performGSR, gsr, 'sctx') #回归信号
...
parcellationList.remove('first_subcortical')
parcellationList.remove('cerebellum')

if noFC!="TRUE":
    #np.save(funcDir+'/surf/'+subject+'_timeseries_subcortical_reg.npy', sctx_corr) 
    for parcellation in parcellationList:
        ...
        uparcel = np.setdiff1d(uparcel, [0, 1042, 2042]) # Remove unnecessary labels
        ts_ctx_cor = np.zeros([data_corr.shape[0], len(uparcel)])
        for lab in range(len(uparcel)):
            tmpData_cor = data_corr[:, thisparc == uparcel[lab]]
            ts_ctx_cor[:,lab] = np.nanmean(tmpData_cor, axis = 1)

        ts_ctx = np.zeros([data.shape[0], len(uparcel)])
        for lab in range(len(uparcel)):
            tmpData = data[:, thisparc == uparcel[lab]]
            ts_ctx[:,lab] = np.nanmean(tmpData, axis = 1)
        
        # get correlation amtrix
        #ts = np.append(sctx_corr, ts_ctx, axis=1)
        np.savetxt(funcDir+'/surf/'+subject+'_atlas-'+parcellation+'_timeseries_cortex.csv',
           ts_ctx, delimiter=',')
        
        np.savetxt(funcDir+'/surf/'+subject+'_atlas-'+parcellation+'_timeseries_cortex_reg.csv',
           ts_ctx_cor, delimiter=',')  # Save cortex time series as CSV file
        #np.save(funcDir+'/surf/'+subject+'_atlas-'+parcellation+'_timeseries_cortex_reg.npy', ts_ctx_cor) # Save cortex time series
