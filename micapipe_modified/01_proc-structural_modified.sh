...
if [ "$PROC" = "qsub-MICA" ] || [ "$PROC" = "qsub-all.q" ] || [ "$PROC" = "LOCAL-MICA" ]; then
    MICAPIPE=/root/autodl-tmp/pipeline/micapipe
...
if [[ "$T1wStr" != "DEFAULT" ]]; then
  ...
  for i in "${!bids_T1wStr[@]}"; do bids_T1wStr[i]=$(ls "${subject_bids}/anat/${idBIDS}_${bids_T1wStr[$i]}.nii.gz"* 2>/dev/null); done
...
if [ ! -f "${proc_struct}/${T1str_nat}".nii.gz ] || [ ! -f "${proc_struct}/${T1str_nat}"_brain.nii.gz ]; then ((N++))
    ...
    for ((k=0; k<=n; k++)); do
      ...
      # Deoblique
      Do_cmd 3dresample -orient LPI -prefix "$t1_obl" -input "${bids_T1ws[k]}"
...
mat_MNI152_SyN_2mm="${dir_warp}/${idBIDS}_from-nativepro_brain_to-MNI152_2mm_mode-image_desc-SyN_"    # transformation strings nativepro to MNI152_2mm
T1_MNI152_InvWarp_2mm="${mat_MNI152_SyN_2mm}1InverseWarp.nii.gz"                      # Inversewarp - nativepro to MNI152_2mm
T1_MNI152_Warp_2mm="${mat_MNI152_SyN_2mm}1Warp.nii.gz"                      # Inversewarp - nativepro to MNI152_2mm
T1_MNI152_affine_2mm="${mat_MNI152_SyN_2mm}0GenericAffine.mat"                        # Affine matrix - nativepro to MNI152_2mm
MNI152_2mm="${util_MNIvolumes}/MNI152_T1_2mm_brain.nii.gz"
MNI152_08mm="${util_MNIvolumes}/MNI152_T1_0.8mm_brain.nii.gz"
MNI152_to_T1nativepro="-t [${T1_MNI152_affine},1] -t ${T1_MNI152_InvWarp}"
T1nativepro_to_MNI152="-t ${T1_MNI152_Warp} -t ${T1_MNI152_affine}"
MNI152_to_T1nativepro_2mm="-t [${T1_MNI152_affine_2mm},1] -t ${T1_MNI152_InvWarp_2mm}"
T1nativepro_to_MNI152_2mm="-t ${T1_MNI152_Warp_2mm} -t ${T1_MNI152_affine_2mm}"


# Save transformations in json file
proc_struct_transformations "$T1nativepro_brain" "${MNI152_08mm}" "${MNI152_2mm}" "${dir_warp}/${idBIDS}_transformations-proc_structural.json" '${T1nativepro_to_MNI152}' '${MNI152_to_T1nativepro}'
...
if [ ! -f "$T1nativepro_maskjson" ]; then ((N++))
    Do_cmd antsApplyTransforms -d 3 -n GenericLabel -i "$MNI152_mask" -r "$T1nativepro_brain" \
            "${MNI152_to_T1nativepro}" -o "$T1nativepro_mask" -v
...
T1_seg_subcortex_1="${dir_volum}/${T1str_atlas}-first_subcortical.nii.gz"
firstout=${T1nativepro_first/.nii.gz/_all_fast_firstseg.nii.gz}

if [ ! -f "$T1_seg_subcortex_1" ] || [ ! -f "$T1nativepro_5tt" ]; then ((N++))
    ...
    if [ -f "$firstout" ]; then Do_cmd cp "$firstout" "$T1_seg_subcortex_1"; ((Nsteps++)); fi
else
    Info "Subject $id has a Subcortical parcellation"; ((Nsteps++)); ((N++))
fi

# use antsApplyTransforms subcortical parcellation to T1-nativepro
unset SGE_ROOT
export FSLPARALLEL=0
T1str_atlas="${idBIDS}_space-nativepro_T1w_atlas"
T1_seg_subcortex="${dir_volum}/${T1str_atlas}-subcortical.nii.gz"
atlas_subcortex="${util_MNIvolumes}/MNI152_T1_2mm_Tian_Subcortex_S1.nii.gz"
#firstout=${T1nativepro_first/.nii.gz/_all_fast_firstseg.nii.gz}

if [ ! -f "$T1_seg_subcortex" ] || [ ! -f "$T1nativepro_5tt" ]; then ((N++))
    Info "Applying ants transformation for subcortical parcellation on T1-nativepro"
    Do_cmd antsApplyTransforms -d 3 -i "$atlas_subcortex" \
                -r "$T1nativepro_brain" \
                -n GenericLabel "${MNI152_to_T1nativepro_2mm}" \
                -o "$T1_seg_subcortex" -v -u int
    if [[ -f "$T1_seg_subcortex" ]]; then ((Nsteps++)); fi
else
    Info "Subject ${id} already has a subcortical parcellation on T1-nativepro"
    ((Nsteps++)); ((N++))
fi
...
if [[ ! -f "$T1_seg_cerebellum" ]]; then ((N++))
    Do_cmd antsApplyTransforms -d 3 -i "$atlas_cerebellum" \
                -r "$T1nativepro" \
                -n GenericLabel "${MNI152_to_T1nativepro}" \
